  ...
  return (
    <>
      <header style={{padding: "5px", margin: "0% 7%"}}>
        ...
        <Button onClick={toggleAuth} variant="contained" style={{float: "right", backgroundColor: "aquamarine", color: "white", border: "none", borderRadius: "3px", marginLeft: "10px"}} >{isAuthenticated ? currentUser.address : "CONNECT"}</Button>
        {isAuthenticated ? <ScoreCell address={currentUser.address} /> : <></>}
      </header>
      {children}
    </>
  )
